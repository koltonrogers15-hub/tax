...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if ($table && $table.length) {
    ...
    $table.bootstrapTable('refreshOptions', { uniqueId: 'state' }); // <-- ensure we can update rows by state name
  }
  ...
  var loadStates = function () {
    ...
    statesLayer.queryFeatures(query)
      .then(function (response) {
        ...
        statesLoaded = true;
        if (maritalLoaded && submitBtn) submitBtn.disabled = false;
      });
  };
  loadStates();
  ...
  var maritalStatusUrl = "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_10_14_Marital_Status_Boundaries/FeatureServer/0";
  const maritalStatusLayer = new FeatureLayer({
  ...
  //Make function to execture query
  // Make function to execute query
  var loadMaritalStatusData = function () {
    ////Create a new query object and give it the correct properties for what you want
    let additionalQuery = maritalStatusLayer.createQuery();
    additionalQuery.returnGeometry = false;
    additionalQuery.outFields = "*"
    ////execute query on your new layer
    maritalStatusLayer.queryFeatures(additionalQuery).then(function (r) {
      console.log(r)
      r.features.forEach(function (feature) {
        ...
        if (!matchKey) {
          console.warn('loadMaritalStatusData: no matching stateObj entry for', abbr, feature.attributes.NAME);
          return;
        }

        // pull marital status metric from actual fields (Percent Married)
        var ms = null;
        if ('B12001_calc_pctMarriedE' in feature.attributes) {
          ms = feature.attributes['B12001_calc_pctMarriedE'];
        } else {
          console.warn('loadMaritalStatusData: B12001_calc_pctMarriedE not found for', abbr, 'attributes keys:', Object.keys(feature.attributes));
        }

        // set maritalStatus (may be null if no field found)
        stateObj[matchKey]['maritalStatus'] = ms;

        // update existing table row in-place so the column stops showing "-"
        $table.bootstrapTable('updateByUniqueId', {
          id: matchKey.replace("_", " "),
          row: { maritalStatus: ms }
        });
      });
      console.log('loadMaritalStatusData: marital status loaded');


$table.bootstrapTable('load', Object.entries(stateObj).map(([k,v]) => ({
  state: k.replace("_"," "),
  maritalStatus: v.maritalStatus
      ...
      console.error('loadMaritalStatusData failed:', err);
      maritalLoaded = true;
      if (statesLoaded && maritalLoaded) {
        if (submitBtn) submitBtn.disabled = false;
      } else {
